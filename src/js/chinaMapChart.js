...
;(function () {
  ...
  axios
    ...
    .then((res) => {
      ...
      document.querySelector('.currentConfirmed_Count').innerHTML =
        currentConfirmed_Count
      // 使用刚指定的配置项和数据显示图表。
      const option = {
        visualMap: [
          {
        ...
        tooltip: {
          formatter: function (params) {
            return `
                    ...
                    累计死亡：${
                      (params.data && params.data.deadCount) || 0
                    } </br>
                    现存确诊：${
                      (params.data && params.data.currentConfirmedCount) || 0
                    }
                    `
          },
        },
